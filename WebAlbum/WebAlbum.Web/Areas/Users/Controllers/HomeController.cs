    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult Index(string condition = null, string searchString = null)
        {
            ...
            try
            {
                ...
                if (condition == "user")
                {
                    data = _mapper.Map<List<Album>, List<SearchAlbumViewModel>>
                    (_albumRepository
                        .AsQueryable()
                        .Where(a => a.User.UserName.ToLower()
                                        .Contains(searchString.ToLower()) && a.Public)
                        .ToList());
                }
                else
                {
                    data = _mapper.Map<List<Album>, List<SearchAlbumViewModel>>(list
                        .Where(a => a.AlbumTitle.ToLower()
                                        .Contains(searchString.ToLower()) && a.Public)
                        .ToList());
                }
            ...
            try
            {
                var photoList = new PhotoListViewModel()
                {
                    ...
                    (_photoRepository.AsQueryable()
                        .Where(u => u.Album.AlbumTitle == albumTitle && u.Album.Public)
                        .OrderBy(p => p.DateCreated).ToList()).ToList()
                };
